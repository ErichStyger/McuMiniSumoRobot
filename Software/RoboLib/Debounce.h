...
typedef enum DBNC_EventKinds {
  DBNC_EVENT_PRESSED,       /*<! Event for key(s) short pressed */
  DBNC_EVENT_RELEASED,      /*<! Event for key(s) short pressed and then released */
  DBNC_EVENT_LONG_PRESSED,  /*<! Event for key(s) pressed for a long time */
  DBNC_EVENT_LONG_RELEASED, /*<! Event for releasing key(s) pressed for a long time */
  DBNC_EVENT_LLONG_PRESSED,  /*<! Event for key(s) pressed for a long-long time */
  DBNC_EVENT_LLONG_RELEASED, /*<! Event for releasing key(s) pressed for a long-long time */
...
typedef struct DBNC_FSMData {
  ...
  uint16_t longlongKeyTicks; /*!< number of trigger ticks needed for long-long key press */
  DBNC_EventKinds lastEvent; /*!< last event */
} DBNC_FSMData;
